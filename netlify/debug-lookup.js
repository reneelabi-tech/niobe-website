...
exports.handler = async function (event) {
  ...
  // Also list all Tamakloe results (smaller set, more likely to contain the target)
  const tamakloeResults = results.filter(r =>
    (r.firstname + ' ' + r.lastname).toLowerCase().includes('tamakloe')
  );
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      WARNING_emmanuel_capped: searchLog.find(s => s.term === 'Emmanuel')?.count === 100
        ? 'Emmanuel search hit 100-result cap — target may be beyond page 1'
        : null,
      phoneMatches:    results.filter(r => r.phoneMatch),
      suspectsByPhone: suspects,
      tamakloeClients: tamakloeResults,
    }, null, 2)
  };
};
